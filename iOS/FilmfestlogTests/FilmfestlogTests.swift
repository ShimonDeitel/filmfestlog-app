import XCTest
@testable import Filmfestlog

@MainActor
final class FilmfestlogTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(FilmEntry(title: "New One"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(FilmEntry(title: "ToDelete"))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.entries = (0..<Store.freeLimit).map { FilmEntry(title: "Item \($0)") }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(FilmEntry(title: "Original"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        store.add(FilmEntry(title: "A"))
        store.add(FilmEntry(title: "B"))
